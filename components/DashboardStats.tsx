...
export default function DashboardStats({ rows, reminderDay }: { rows: Row[]; reminderDay: number }) {
    ...
    <div>
      ...
      <div className="card">
        ...
        <div className="table-wrap">
          <table>
            <thead><tr><th>לקוח</th><th>עובדת אחראית</th><th>התקדמות</th>{filter === 'behind' && <th>ימי פיגור</th>}</tr></thead>
            <tbody>
              {filtered.map(r => (
              ...
              {filtered.length === 0 && <tr><td colSpan={filter === 'behind' ? 4 : 3} className="td-muted">{emptyLabels[filter]}</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
